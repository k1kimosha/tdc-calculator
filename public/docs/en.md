...
6. **Export/import** — “Export JSON” / “Import JSON” save and restore the whole log. Each shot in the file carries the blocks of every calculator, including the used formulas. The log is not touched by “Reset data” in the reference; use **“Clear log”** to wipe it completely.
7. **PDF report** — after the review press **“Print report”** on a patrol card: the PDF document is generated by the pdfmake library and downloaded by the browser. The report includes the outcome summary and a separate block per shot — time, calculator with its variables, formulas and results, the outcome and the note; plus patrol times, U-boat and author nick.
...
- **U-boat selection** instead of a ship when starting a patrol: U-96, U-564, U-552, U-307.
- **Shot snapshot** now covers all calculators at once: range, speed and AOB are recorded together, and the **formulas** used are saved with the record, exported to JSON and included in the PDF report.
- **PDF report** is built with the pdfmake library and downloaded as a file instead of the browser print dialog; the shots table now lists the used formulas.
- Fixed the lead-to-passing-distance formula display on the speed calculator.

### 1.3

- Added the **“Patrol log”** section: starting the stopwatch, recording shots from the calculators, ending a patrol, reviewing torpedo outcomes, author nick, log export/import and PDF report printing.
